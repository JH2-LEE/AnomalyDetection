...
# df.to_csv("logging/collision.csv", index=False, columns=["frame", "intensity"])
...
class World(object):
    ...
    def _get_data_from_carla(self):
        ...
        try:
            ...
            if self.args.map is None:
                ...
                world = self.client.load_world(self.args.map, True)
    ...
    def start(self, hud, input_control):
        ...
        ## added
        pth = self.client.start_recorder("accident0418_1.log", True)
        self.world.recording_enabled = True
        print("recording start", pth)
        ...
        pygame.draw.circle(
            ...
            COLOR_ALUMINIUM_1,
            center_offset,
            int(self._hud.dim[1] / 2),
        )
        pygame.draw.circle(
            self.border_round_surface,
            COLOR_WHITE,
            center_offset,
            int((self._hud.dim[1] - 8) / 2),
        )
        ...
        # --------------
        # Add collision sensor to ego(hero) vehicle.
        # --------------

        col_bp = self.world.get_blueprint_library().find("sensor.other.collision")
        col_location = carla.Location(self.hero_actor.bounding_box.extent.x, 0, 0)
        col_rotation = carla.Rotation(0, 0, 0)
        col_transform = carla.Transform(col_location, col_rotation)
        ego_col = self.world.spawn_actor(
            col_bp,
            col_transform,
            attach_to=self.hero_actor,
            attachment_type=carla.AttachmentType.Rigid,
        )
        self.spawned_sensors.append(ego_col)
        ...
        def col_callback(colli):
            ...
            self.col_list.append([colli.frame, intensity])

        ego_col.listen(lambda colli: col_callback(colli))

        # --------------
        # Add a RGB camera sensor to ego(hero) vehicle.
        # --------------

        cam_rgb_bp = None
        cam_rgb_bp = self.world.get_blueprint_library().find("sensor.camera.rgb")
        cam_rgb_location = carla.Location(1.5, 0, 2.4)
        cam_rgb_rotation = carla.Rotation(0, 0, 0)
        cam_rgb_transform = carla.Transform(cam_rgb_location, cam_rgb_rotation)
        ego_cam_rgb = self.world.spawn_actor(
            cam_rgb_bp,
            cam_rgb_transform,
            attach_to=self.hero_actor,
            attachment_type=carla.AttachmentType.Rigid,
        )
        self.spawned_sensors.append(ego_cam_rgb)
        c_raw = carla.ColorConverter.Raw
        ego_cam_rgb.listen(
            lambda image: image.save_to_disk(
                "logging/rgb/%.6d.png" % image.frame, c_raw
            )
        )

        # --------------
        # Add a depth camera to ego(hero) vehicle
        # --------------
        cam_depth_bp = None
        cam_depth_bp = self.world.get_blueprint_library().find("sensor.camera.depth")
        cam_depth_location = carla.Location(1.5, 0, 2.4)
        cam_depth_rotation = carla.Rotation(0, 0, 0)
        cam_depth_transform = carla.Transform(cam_depth_location, cam_depth_rotation)
        ego_cam_depth = self.world.spawn_actor(
            cam_depth_bp,
            cam_depth_transform,
            attach_to=self.hero_actor,
            attachment_type=carla.AttachmentType.Rigid,
        )
        ego_cam_depth02 = self.world.spawn_actor(
            cam_depth_bp,
            cam_depth_transform,
            attach_to=self.hero_actor,
            attachment_type=carla.AttachmentType.Rigid,
        )
        self.spawned_sensors.append(ego_cam_depth)
        self.spawned_sensors.append(ego_cam_depth02)
        c_log_depth = carla.ColorConverter.LogarithmicDepth
        c_depth = carla.ColorConverter.Depth
        ego_cam_depth.listen(
            lambda image: image.save_to_disk(
                "logging/log_depth/%.6d.png" % image.frame, c_log_depth
            )
        )
        ego_cam_depth02.listen(
            lambda image: image.save_to_disk(
                "logging/raw_depth/%.6d.png" % image.frame, c_depth
            )
        )

        # --------------
        # Add a semantic segmentation camera to ego(hero) vehicle
        # --------------
        ...
        sem_location = carla.Location(1.5, 0, 2.4)
        sem_rotation = carla.Rotation(0, 0, 0)
        sem_transform = carla.Transform(sem_location, sem_rotation)
        sem_cam = self.world.spawn_actor(
            sem_bp,
            sem_transform,
            attach_to=self.hero_actor,
            attachment_type=carla.AttachmentType.Rigid,
        )
        self.spawned_sensors.append(sem_cam)
        # This time, a color converter is applied to the image, to get the semantic segmentation view
        sem_cam.listen(
            lambda image: image.save_to_disk(
                "logging/semseg/%.6d.png" % image.frame,
                carla.ColorConverter.CityScapesPalette,
            )
        )

        # --------------
        # Add a DVS camera to ego(hero) vehicle
        # --------------
        ...
        cam_dvs_bp = None
        cam_dvs_bp = self.world.get_blueprint_library().find("sensor.camera.dvs")
        cam_dvs_location = carla.Location(1.5, 0, 2.4)
        cam_dvs_rotation = carla.Rotation(0, 0, 0)
        cam_dvs_transform = carla.Transform(cam_dvs_location, cam_dvs_rotation)
        ego_cam_dvs = self.world.spawn_actor(
            cam_dvs_bp,
            cam_dvs_transform,
            attach_to=self.hero_actor,
            attachment_type=carla.AttachmentType.Rigid,
        )
        self.spawned_sensors.append(ego_cam_dvs)
        if not os.path.exists("logging/dvs"):
            os.makedirs("logging/dvs")
        ego_cam_dvs.listen(
            lambda image: Image.fromarray(dvs_cvt(image)).save(
                "logging/dvs/%06d.png" % image.frame
            )
        )

        # --------------
        # Add LIDAR sensor to ego(hero) vehicle.
        # --------------
        lidar_bp = None
        ...
        lidar_sen = self.world.spawn_actor(
            lidar_bp, lidar_transform, attach_to=self.hero_actor
        )
        self.spawned_sensors.append(lidar_sen)
        lidar_sen.listen(
            lambda point_cloud: point_cloud.save_to_disk(
                "logging/lidar/%.6d.ply" % point_cloud.frame
            )
        )

        # --------------
        # Add a optical flow camera to ego(hero) vehicle
        # --------------
        def opt_cvt(image):
        ...
        cam_optical_flow_bp = None
        cam_optical_flow_bp = self.world.get_blueprint_library().find(
            "sensor.camera.optical_flow"
        )
        cam_optical_flow_location = carla.Location(1.5, 0, 2.4)
        cam_optical_flow_rotation = carla.Rotation(0, 0, 0)
        cam_optical_flow_transform = carla.Transform(
            cam_optical_flow_location, cam_optical_flow_rotation
        )
        ego_cam_optical_flow = self.world.spawn_actor(
            cam_optical_flow_bp,
            cam_optical_flow_transform,
            attach_to=self.hero_actor,
            attachment_type=carla.AttachmentType.Rigid,
        )
        self.spawned_sensors.append(ego_cam_optical_flow)
        if not os.path.exists("logging/optical_flow"):
            os.makedirs("logging/optical_flow")
        ego_cam_optical_flow.listen(
            lambda image: Image.fromarray(opt_cvt(image)).save(
                "logging/optical_flow/%06d.png" % image.frame
            )
        )
        # ego_cam_optical_flow.listen(
        #     lambda image: image.save_to_disk(
        #         "logging/opotical_flow/%.6d.jpg" % image.frame, c_raw
        #     )
        # )
    ...
    def _spawn_hero(self):
        ...
        blueprint = random.choice(
            self.world.get_blueprint_library().filter(self.args.filter)
        )
    ...
    def destroy(self):
        ...
        ## added
        print("hero id", self.hero_actor.id)
        ...
        df = pd.DataFrame(data=self.col_list, columns=["frame", "intensity"])
        df.to_csv("logging/collision.csv", index=False)
...
def main():
    ...
    argparser.add_argument(
        "--no-rendering", action="store_true", help="switch off server rendering"
    )
    argparser.add_argument(
        "--show-triggers",
        action="store_true",
